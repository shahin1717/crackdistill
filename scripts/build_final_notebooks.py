...
def main():
    ...
    # 6. OOD & Tiled Inference Notebook
    nb6 = generate_ood_tiled_eval_notebook()
    with open(final_dir / "06_eval_ood_and_tiled_inference.ipynb", "w") as f:
        json.dump(nb6, f, indent=1, ensure_ascii=False)
    print("✓ Created final_notebooks/06_eval_ood_and_tiled_inference.ipynb")
    
    # 7. Benchmark Speed Notebook
    nb7 = generate_benchmark_notebook()
    with open(final_dir / "07_benchmark_speed_and_profile.ipynb", "w") as f:
        json.dump(nb7, f, indent=1, ensure_ascii=False)
    print("✓ Created final_notebooks/07_benchmark_speed_and_profile.ipynb")
...
| **`06_eval_ood_and_tiled_inference.ipynb`** | **OOD & Tiled Inference Engine**: Evaluates checkpoints on uncropped images with direct resizing vs tiled sliding window ($512 \\times 512$ native patches). | ~5–10 mins | `results/ood_eval_summary.json` |
| **`07_benchmark_speed_and_profile.ipynb`** | **Speed Benchmark**: Confirms 0% latency/parameter overhead (>100 FPS, 2.84M params, 10.2 GFLOPs). | ~2 mins | Latency & FPS Report |

---

## ⚙️ Execution Guide in Kaggle

1. **Upload Notebook**: In Kaggle, click **New Notebook** $\\\\rightarrow$ **File** $\\\\rightarrow$ **Import Notebook** $\\\\rightarrow$ Select desired notebook.
2. **Session Settings**:
   * **Accelerator**: `GPU T4 x2` or `GPU P100`
   * **Internet**: `ON`
3. **Attach Dataset**:
   * Click **+ Add Data** $\\\\rightarrow$ attach `distill_datasetforme` (or Crack500 dataset).
4. **Click "Run All"**:
   * Automatically verifies teacher logits ($N > 0$), trains for 150 epochs, runs in-domain and OOD evaluations, and exports JSON summaries.
"""
